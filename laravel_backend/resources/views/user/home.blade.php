...
<div class="table-card" style="margin-top: 1.5rem;">
    <div class="table-header">
        <div>
            ...
            <div class="table-subtitle">Perbandingan rata-rata harga komoditas berdasarkan kategori.</div>
        </div>
    </div>
...
(function () {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: (ctx) => {
                            const val = ctx.parsed.toLocaleString('id-ID');
                            return ` Rp ${val}`;
                        }
                    }
                }
    ...
    labels.forEach((label, i) => {
        ...
        el.innerHTML = `
            ...
            <span style="flex:1;color:var(--text-muted, #6b7280);">${label}</span>
            <span style="font-weight:600;color:var(--text-primary, #111);">Rp ${val}</span>
        `;
        legend.appendChild(el);
    });
})();

/* Intercept filter bar submit — kalau semua field kosong, redirect ke URL bersih */
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const semuaKosong = !search && !category && !date;

        if (semuaKosong) {
            e.preventDefault();
            // Buang semua query string, kembali ke URL bersih
